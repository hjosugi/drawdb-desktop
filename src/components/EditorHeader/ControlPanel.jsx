...
const ConfigureCustomTypes = lazy(() => import("./ConfigureCustomTypes"));
...
export default function ControlPanel({
  ...
  const desktop = useDesktopFileMenu({
    diagramId,
    setLastSaved,
    setTitle,
    title,
  ...
  const save = async () => {
    ...
    if (desktop.available) {
      // Save the external file first. Triggering the upstream Dexie save before
      // a new .ddb receives its ID can create two competing diagram records.
      if (await desktop.saveDdb()) setSaveState(State.SAVING);
      return;
    }
  ...
  const open = desktop.available ? desktop.openDdb : () => setModal(MODAL.OPEN);
  const saveDiagramAs = desktop.available
    ? () => desktop.saveDdb({ saveAs: true })
    ...
    toastId = Toast.success({
      ...
      content: (
        <span>
          ...
          <Typography.Text
            link={{
              href: appRouteUrl(`/editor/diagrams/${newId}${window.location.search}`),
              target: "_blank",
  ...
  const menu = {
    file: {
      ...
      new_window: {
        function: () => window.open(appRouteUrl("/editor"), "_blank"),
      },
      open: {
        function: open,
        shortcut: "Ctrl+O",
    ...
    help: {
      docs: {
        function: () => openExternalUrl(socials.docs),
        shortcut: "Ctrl+H",
      },
      shortcuts: {
        function: () => openExternalUrl(`${socials.docs}/shortcuts`),
      },
      ask_on_discord: {
        function: () => openExternalUrl(socials.discord),
      },
      report_bug: {
        function: () => desktop.available
          ? openExternalUrl(`${socials.github}/issues/new/choose`)
          : window.open(appRouteUrl("/bug-report"), "_blank"),
      },
    },
  };
  ...
  useHotkeys("mod+h", () => openExternalUrl(socials.docs), {
    preventDefault: true,
  });
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>
        {modal !== MODAL.NONE && modal !== MODAL.CONFIG_CUSTOM_TYPES ? (
          <Modal
            modal={modal}
                          ...
                          return (
                            ...
                            >
                              <div className="w-full flex items-center justify-between">
                                <div>{menu[category][item].name ?? t(item)}</div>
