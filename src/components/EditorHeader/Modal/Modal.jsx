...
const ImportDiagram = lazy(() => import("./ImportDiagram"));
...
export default function Modal({
  ...
  const getModalOnOk = async () => {
    switch (modal) {
      ...
      case MODAL.SAVEAS:
        ...
        return;
      case MODAL.NEW:
        window.open(appRouteUrl(`/editor/templates/${selectedTemplateId}`), "_blank");
        setModal(MODAL.NONE);
        return;
      case MODAL.LANGUAGE:
  ...
  return (
    ...
    >
      <Suspense
        fallback={(
          <div className="my-3 text-center text-sky-600">
            <Spin tip={t("loading")} size="large" />
          </div>
        )}
      >
