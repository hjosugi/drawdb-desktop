...
credentials and are not claimed by CI.
...
for WebKitGTK, GTK, AppIndicator, and librsvg.

## Linux distribution policy

| Channel | Policy |
| --- | --- |
| RPM | Supported as a first-party GitHub Release artifact. Test install with `sudo dnf install ./drawDB-*.rpm`, launch `drawDB`, then uninstall with `sudo dnf remove drawdb`. |
